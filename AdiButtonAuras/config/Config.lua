...
AdiButtonAuras:CreateConfig(function(addonName, addon)
	...
	function addon:OpenConfiguration(what)
		...
		if panels[what] then
			--return _G.Settings.OpenToCategory(addon.categoryID) -- todo
			return _G.Settings.OpenToCategory(panels[what].name)
		else
			return _G.Settings.OpenToCategory(panels.main.name) -- LibSpellbook seems broken, just default to main if no panel was specified
		end
		...
		if key and addon.spells[key] then
			_G.Settings.OpenToCategory(panels.spells.name)
