...
function overlayPrototype:UpdateGUID(event, unit)
	...
	if not issecretvalue(guid) and self.guids[unit] ~= guid then
...
function overlayPrototype:UpdateState(event)
	...
	if self.handlers then
		...
		if missing ~= "none" and false then -- DEBUG: disabled for now, as it would show in combat for things that are actually present
