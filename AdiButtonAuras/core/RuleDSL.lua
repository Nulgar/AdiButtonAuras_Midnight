...
local function GetHighlightHandler(highlight)
	...
	if highlight == "good" or highlight == "bad" or highlight == "darken" or highlight == "lighten" then
		return function(model, count, expiration)
			...
			if issecretvalue(count) then print("secret count:",count,expiration) end
			if not issecretvalue(count) and count and count > 1 then
				model.count = count
			end
			if expiration then
...
local function BuildAuraHandler_Longest(filter, highlight, token, buffs, callLevel)
	...
	return function(units, model)
		...
		for _, id, count, expiration in IterateAuras(unit) do
			if not issecretvalue(id) and buffs[id] --and expiration > longest
			then
				longest = expiration
				Show(model, count, expiration)
			end
...
local function BuildAuraHandler_FirstOf(filter, highlight, token, buffs, callLevel)
	...
	return function(units, model)
		...
		if not unit or unit == '' then return end
		for _, id, count, expiration in IterateAuras(unit) do
			if not issecretvalue(id) and buffs[id] then
				Show(model, count, expiration)
				return true
			end
		end
...
local function BuildDispelHandler(filter, highlight, token, dispellable, callLevel)
	...
	return function(units, model)
		local unit = units[token]
		if not unit or unit == '' then return end
		for a, b, count, expiration, dispel in IterateAuras(unit) do
			if not issecretvalue(dispel) and dispellable[dispel] then
				--print(a, b, count, expiration, dispel)
...
local function BuildTemporaryPetHandler(guid, highlight)
	return function (_, model)
		...
		if pet and pet:match('%-' .. guid .. '%-') then
			...
			if not issecretvalue(remaining) and remaining then
...
local function BuildTotemHandler(totemTexture, highlight)
	return function (_, model)
		for slot = 1, 6 do
			...
			if not issecretvalue(found) and found and texture == totemTexture then
...
local function ShowCountAndHighlight(key, spells, unit, events, handler, highlight, desc, descWhat, getValue, getMax, providers)
	...
	local highlightHandler = function(units, model)
		...
		--print("actualUnit",issecretvalue(actualUnit),"maxi", issecretvalue(maxi),"value",issecretvalue(value) )
		if issecretvalue(value) or maxi == 0 or not test(value, maxi) then return end
		showHighlight(model)
		return true
	end
...
local function ShowPower(spells, powerType, handler, highlight, providers, desc)
	...
	if type(powerType) == "string" then
		...
		if type(powerLoc)=="table" then powerLoc = powerLoc.name end
...
do
	...
	local function WarnOnStaleData(category)
		...
		if false and math.floor(patch / 1e4) < math.floor(build / 1e4) then
			print(
				format(
...
local RULES_ENV = addon.BuildSafeEnv(
	...
	-- Allowed globals
	{
		...
		"select", "string", "table", "tinsert", "type", "issecretvalue", "scrubsecretvalues",
		-- WoW API
		...
		"UnitStagger",
		-- custom
		"C_UIWidgetManager", "GetInventoryItemLink", "floor", "GetTime", "GetSpellName", "C_Spell", "UnitInPartyIsAI",
	}
)
