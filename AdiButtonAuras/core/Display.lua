...
local function Timer_Update(self)
	local timeLeft = (scrubsecretvalues(self.expiration) or 0) - GetTime()
	if timeLeft <= 0 then
		self:Hide()
		return
	end
...
function overlayPrototype:LayoutTexts()
	...
	local text = parentCount:GetText()
	text = issecretvalue(text) and "" or text or ""
	local parentCountIsShown = parentCount:IsShown() and strtrim(text or "") ~= ""
	if parentCountIsShown and count:IsShown() and text == count:GetText() then
		return count:Hide()
	end
...
function overlayPrototype:SetExpiration(expiration)
	if not issecretvalue(expiration) then
...
function overlayPrototype:SetCount(count, maxCount)
	count = scrubsecretvalues(count) -- if secret, just drop it completely (turns into nil) rather than dropping the check for 0 below
	count = tonumber(count)
	maxCount = tonumber(scrubsecretvalues(maxCount)) -- can be secret somehow?
...
ScaleDown = AbbreviateLargeNumbers -- use Blizzard function instead - it has additional parameters to set breakpoints, but can't figure them out

function overlayPrototype:ApplyCount()
	local count = scrubsecretvalues(self.count)
	local maxCount = self.maxCount
	if --not issecretvalue(count) and
	count then
