...
local function ProcessAura(data, aura)
	if not data then return aura end -- happened once, for whatever reason
	-- try to remove secret flags - if value is secret, scrubsecretvalues returns nil
	aura.count = scrubsecretvalues(data.applications) or data.applications
	aura.dispel = scrubsecretvalues(data.dispelName) == '' and 'Enrage' or scrubsecretvalues(data.dispelName) or data.dispelName
	aura.expiration = scrubsecretvalues(data.expirationTime) or data.expirationTime
	aura.id = scrubsecretvalues(data.spellId) or data.spellId
...
local aurasMetatable = {
	__index = {
		CheckGUID = function (self)
			local guid = UnitGUID(self.__unit)
			if not issecretvalue(self.__guid) and not issecretvalue(guid) and self.__guid ~= UnitGUID(self.__unit) then
				self:Update()
			end
		...
		Update = function (self, info)
			...
			--if issecretvalue(self.__guid) then self.__guid = nil end
			if not issecretvalue(self.__guid) and not self.__guid then
		...
		IncrementalUpdate = function (self, unit, filter, info)
			...
			for _, auraInstanceID in next, info.removedAuraInstanceIDs or empty do
				local aura = rawget(self, auraInstanceID)

				if aura then
					self[auraInstanceID] = del(aura)
				end
			end
		end,
		GetById = function (self, id)
			for k, v in next, self do
				if type(k) == 'number' and not issecretvalue(v.id) and v.id == id then
					return v
				end
			end
		end,
...
for key in next, mts do
	getters[key] = function(unit, id)
		if --not issecretvalue(unit) and not issecretvalue(id) and
		unit and UnitExists(unit) then
	...
	iterators[key] = function(unit)
		if --issecretvalue(unit) or
		not unit or not UnitExists(unit) then return NOP end
