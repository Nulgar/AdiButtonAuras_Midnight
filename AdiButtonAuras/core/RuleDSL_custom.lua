...
local function CheckAvailability(info, spellId, providers)
	-- custom
	if NulgarAdiSpells and NulgarAdiSpells[spellId] then
		--print("override",C_Spell.GetSpellName(spellId),LibSpellbook:IsKnown(spellId))
		return true
	end
...
local function PassiveModifier(passive, spell, buff, unit, highlight)
	...
	local handler = BuildAuraHandler_Single("HELPFUL", highlight, unit, buff, 3)
	local key = BuildKey("PassiveModifier", passive, spell, buff, unit, highlight)
	local desc = BuildDesc("HELPFUL", highlight, unit, buff)
...
local function ShowPower(spells, powerType, handler, highlight, providers, desc)
	...
	if type(powerType) == "string" then
		...
		--C_Timer.After(1,function()print(powerType,powerKey,_G[powerKey],_G['POWER_TYPE_'..powerKey])end)
...
local function ShowHealth(spells, unit, handler, highlight, providers, desc, percent)
	return ShowCountAndHighlight(
		...
		desc,
		_G.HEALTH,
		percent and function(unit) return floor(UnitHealth(unit)/UnitHealthMax(unit)*100) end or UnitHealth,
		percent and 100 or UnitHealthMax,
		providers
	)
end
...
local baseEnv = {
	...
	-- custom
	NAuraAliases = WrapTableArgFunc(AuraAliases),
